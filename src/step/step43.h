...
void step43()
{
    ...
    auto w1 = variable<double>("W1", nw1);
    auto b1 = variable<double>("b1", nb1);
    auto w2 = variable<double>("W2", nw2);
    auto b2 = variable<double>("b2", nb2);
    ...
    {
        auto y_pred = linear("l1",x, w1, b1);
        y_pred = sigmoid("sig", y_pred);
        y_pred = linear("l2", y_pred, w2, b2);
        auto loss = mean_squared_error("mse", y, y_pred);

        //qDebug() << "y_pred:";
        //qDebug() << y_pred.fowardText().c_str();
        ...
        auto dw1 = w1.grad()->_data * lr;
        auto db1 = b1.grad()->_data * lr;
        auto dw2 = w2.grad()->_data * lr;
        auto db2 = b2.grad()->_data * lr;

        auto w1_new = w1.data() - dw1;
        auto b1_new = b1.data() - db1;
        auto w2_new = w2.data() - dw2;
        auto b2_new = b2.data() - db2;
        ...
        if( i%1000 == 0 )
        {
            ...
            qDebug() << " +++ data ++++";
            qDebug() << " W1:" << w1.dataText().c_str();
            qDebug() << " b1:" << b1.dataText().c_str();
            qDebug() << " W2:" << w2.dataText().c_str();
            qDebug() << " b2:" << b2.dataText().c_str();
        }

        /*
        qDebug() << " +++ grad ++++";
        qDebug() << " W1:" << w1.gradText().c_str();
        qDebug() << " b1:" << b1.gradText().c_str();
        qDebug() << " W2:" << w2.gradText().c_str();
        qDebug() << " b2:" << b2.gradText().c_str();*/
        //loss.showData();
    }
